...
const TVCard = memo(({ movie: tvShow, onDelete, onEdit, showAvgRating = true }) => {
    ...
    const typeConfig = {
        ...
        Series: 'text-[#5daeff] border-[#5daeff]/35',
    }

    let statusKey = tvShow.status
    if (statusKey === 'playing') {
        statusKey = 'watching'
    }
    const sc = (statusKey && statusConfig[statusKey]) ? statusConfig[statusKey] : null

    const imageUrl = tvShow.cover || tvShow.coverImage

    const handleCardClick = () => {
        if (tvShow.externalId) {
            navigate(`/tv/${tvShow.externalId}`)
        }
    }
