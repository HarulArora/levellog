...
// ── Search Bar ──
function AnimeSearchBar({ id = 'anime-search', section }) {
    const navigate = useNavigate()
    ...
    const handleChange = (e) => {
        ...
        debounceRef.current = setTimeout(async () => {
            try {
                setLoading(true)
                setOpen(true)
                const res = await api.get(`/anime/search?q=${encodeURIComponent(val)}&type=${type}&limit=10`)
            ...
            {open && (
                <div className="absolute top-[calc(100%+6px)] left-0 right-0 z-[60] bg-[#111118] border border-[#2a2a35] rounded-xl shadow-2xl overflow-hidden">
                    {results.length > 0 ? (
                        ...
                        <div className="px-4 py-5 text-center font-mono text-xs text-[#7a7a90]">No anime found</div>
                    ) : (
...
function AnimeHome() {
    ...
    return (
        <div className="min-h-screen">
            <Helmet>
                <title>QuestDuck | Track Anime</title>
            </Helmet>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                        <div>
                            ...
                            <p className="text-[#a0a0b8] text-sm leading-relaxed mb-8 max-w-md">
                                Track everything you watch. From seasonal hits to all-time classics. 
                                Log episodes, rate series, and see what's trending.
                            </p>
                                ...
                                {user ? (
                                    <>
                                        <button onClick={() => setShowAddModal(true)} className="btn-apple btn-apple-primary px-6 py-3 gap-1.5">
                                            <Plus size={16} strokeWidth={2.5} /> Log Anime
                                        </button>
                                        <button onClick={() => navigate('/anime/library')} className="btn-apple btn-apple-secondary px-6 py-3 gap-1.5">
                                            My Library →
                                        </button>
                                    </>
                                ) : (
                            ...
                            {recentAnime.length > 0 ? (
                                <>
                                    ...
                                    {(recentAnime || []).map(item => {
                                        ...
                                        return (
                                            <div key={item._id} onClick={() => navigate(`/anime/${item.externalId}`)} className="flex items-center gap-4 bg-[#111118]/80 border border-[#2a2a35] rounded-lg p-3 hover:border-[#c8ff57]/30 transition-all cursor-pointer">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        ...
                                                        <span className="font-mono text-[9px] uppercase tracking-wider px-2 py-1 rounded-sm bg-[#2a2a35] text-[#94999c]">Anime</span>
                                                    </div>
                                                </div>
                                                {item.rating > 0 && (
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col gap-1 min-w-0">
                                    ...
                                    <div className="font-mono text-[10px] text-[#7a7a90]">@{user.username} · Member</div>
                        ...
                        (sections || []).map(section => (
                            <div key={section.title}>
                                <div className="flex items-center justify-between mb-8 group">
                                    ...
                                    <div 
                                        onClick={() => navigate('/anime/discover')}
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                                        {(section.items || []).slice(0, 12).map(item => (
                                            <AnimeCard 
                                                key={`${section.title}-${item.externalId}`} 
