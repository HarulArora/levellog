...
import { Plus, List as ListIcon, Heart, Bookmark, Search, MoreVertical, Trash2, Edit2, Share2, ChevronRight, Grid, LayoutGrid, ListChecks, Tv, BookOpen } from 'lucide-react'
...
const ListItemCard = memo(({ item, onRemove, section }) => {
    ...
    return (
        <div className="group relative bg-[#111118] border border-[#2a2a35] rounded-xl overflow-hidden hover:border-[#c8ff57] transition-all duration-300">
            <div className="aspect-[3/4] relative overflow-hidden cursor-pointer" onClick={() => navigate(`/anime/${item.externalId}?type=${section}`)}>
            ...
            <div className="p-3">
                ...
                <div className="font-mono text-[9px] text-[#7a7a90] mt-1 uppercase tracking-wider">{item.genre || 'Anime'}</div>
            </div>
        </div>
    )
})
...
function AnimeLists() {
    ...
    const handleCreateList = async () => {
        ...
        try {
            await api.post('/lists/custom', { ...newListData, mediaType: 'anime' })
    ...
    const filteredLiked = useMemo(() => likedAnime.filter(a => (a.type || a.mediaType) === 'anime'), [likedAnime])
    const filteredWishlist = useMemo(() => wishlist.filter(a => (a.type || a.mediaType) === 'anime'), [wishlist])
    const filteredLists = useMemo(() => myLists.filter(l => l.mediaType === 'anime'), [myLists])
    ...
    return (
        <div className="max-w-[1200px] mx-auto px-5 md:px-10 py-24 min-h-screen">
            <Helmet>
                <title>My Anime Collections | QuestDuck</title>
            </Helmet>
                    ...
                    <div>
                        ...
                        {selectedList.items.length > 0 ? (
                            ...
                            <EmptyState 
                                ...
                                description="Search and add anime to this collection from their detail pages."
                                actionLabel="Browse Trending"
                                onAction={() => navigate('/anime/discover')}
                            />
                        )}
                    </div>
                ) : (
                                ...
                                {filteredLists.length === 0 && !loading && (
                                    <div className="mt-8">
                                        <EmptyState 
                                            ...
                                            description="Group your favorite anime into custom collections like 'Hidden Gems' or 'Must Watch'."
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                {filteredLiked.length > 0 ? (
                                    ...
                                    <EmptyState 
                                        ...
                                        description="Anime you ❤️ will appear here for quick access."
                            ...
                            <div>
                                {filteredWishlist.length > 0 ? (
                                    ...
                                    <EmptyState 
                                        ...
                                        description="Keep track of upcoming releases or anime you want to watch later."
