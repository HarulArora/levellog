...
function AnimeLibrary() {
    ...
    const filteredAnime = useMemo(() => {
        return library
            .filter(a => (a.type || a.mediaType) === 'anime')
    ...
    return (
        <div className="min-h-screen pb-32">
            <Helmet>
                <title>My Anime Library | QuestDuck</title>
            </Helmet>
                                ...
                                <tbody>
                                    {filteredAnime.map((anime, idx) => (
                                        <AnimeRow 
                                            ...
                                            anime={anime} 
                                            index={idx + 1}
                                            onDelete={() => handleDelete(anime._id)}
                    ...
                    <div className="py-32 text-center border-2 border-dashed border-[#2a2a35] rounded-[40px] bg-[#111118]/30 backdrop-blur-sm relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-[#7a7a90] font-mono text-xs uppercase tracking-widest max-w-xs mx-auto leading-relaxed">
                                {searchQuery ? `No results for "${searchQuery}". Maybe try a different keyword?` : 'Your anime journey begins with a single log.'}
                            </p>
                            {!searchQuery && (
                                <button 
...
function AnimeRow({ anime, index, onDelete, onEdit }) {
    const navigate = useNavigate()
    const statusConfig = {
    ...
    const sc = statusConfig[anime.status] || statusConfig.planned
