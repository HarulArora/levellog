...
const CommentItem = memo(({ comment, currentUser, externalId, type, onRefresh, onXpToast, setAllComments, depth = 0, title = '' }) => {
    ...
    const handleDelete = async () => {
        try {
            setSubmittingReply(true) // Reuse submitting state for loading
                        ...
                        {activeTab === 'overview' && (
                            <>
                                ...
                                {(() => {
                                    ...
                                    // Logic: Prefer TV series over movies/specials for the "main" timeline
                                    const prequel = prequelItems.find(i => i.type?.toLowerCase() === 'tv') || prequelItems[0];
