...
const getProviderLink = (name, title, fallback) => {
    const cleanName = name.toLowerCase();
    const query = encodeURIComponent(title);
    if (cleanName.includes('netflix')) return `https://www.netflix.com/search?q=${query}`;
    if (cleanName.includes('amazon') || cleanName.includes('prime')) return `https://www.amazon.com/s?k=${query}&i=instant-video`;
    if (cleanName.includes('disney')) return `https://www.disneyplus.com/search?q=${query}`;
    if (cleanName.includes('hulu')) return `https://www.hulu.com/search?q=${query}`;
    if (cleanName.includes('crunchyroll')) return `https://www.crunchyroll.com/search?q=${query}`;
    if (cleanName.includes('apple')) return `https://tv.apple.com/search?term=${query}`;
    if (cleanName.includes('google')) return `https://play.google.com/store/search?q=${query}&c=movies`;
    return fallback;
};
...
function AnimeDetail() {
    ...
    const handleLike = async () => {
        ...
        try {
            const res = await api.post('/anime/like', { externalId: id, title: anime.title, cover: anime.cover || anime.coverImage, type, genre: anime.genres?.[0] })
    ...
    const handleWishlist = async () => {
        ...
        try {
            const res = await api.post('/anime/wishlist', { externalId: id, title: anime.title, cover: anime.cover || anime.coverImage, type, genre: anime.genres?.[0] })
                        ...
                        {activeTab === 'watch' && (
                            <div className="flex flex-col gap-6">
                                {(() => {
                                    ...
                                    const availableRegions = Object.keys(anime.watchProviders || {})
                                        .filter(reg => reg !== 'link' && anime.watchProviders[reg] && (anime.watchProviders[reg].flatrate || anime.watchProviders[reg].rent || anime.watchProviders[reg].buy));
                                    ...
                                    const currentReg = anime.watchProviders?.[watchRegion] ? watchRegion : (availableRegions[0] || 'IN');
                                    const providers = anime.watchProviders?.[currentReg];
                                    ...
                                    return (
                                        <>
                                            ...
                                            {(!hasData || availableRegions.length === 0) ? (
                                                <div className="bg-[#111118] border border-[#2a2a35] rounded-xl p-12 text-center shadow-xl">
                                                    ...
                                                    <div className="text-[#7a7a90] font-mono text-xs uppercase tracking-widest mb-6">Official Streaming Links</div>
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        {anime.streamingLinks?.map((link, i) => (
                                                               ...
                                                               className="flex items-center justify-between p-5 rounded-2xl bg-[#18181f]/40 border border-white/5 hover:border-[#c8ff57]/40 hover:bg-[#18181f]/60 transition-all group shadow-xl text-left">
                                                                <div className="flex items-center gap-4">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <div className="text-[#7a7a90] text-[10px] font-mono uppercase tracking-[0.2em] mt-0.5">Official Stream</div>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <div className="bg-[#111118] border border-[#2a2a35] rounded-lg p-6">
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                {providers.flatrate.map((p, i) => (
                                                                    <a key={i} href={getProviderLink(p.provider_name, anime.title, providers.link)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 rounded-xl bg-white/5 border border-white/5 hover:border-[#c8ff57]/20 transition-all group cursor-pointer">
                                                        ...
                                                        <div className="bg-[#111118] border border-[#2a2a35] rounded-lg p-6">
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                {providers.rent.map((p, i) => (
                                                                    <a key={i} href={getProviderLink(p.provider_name, anime.title, providers.link)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 rounded-xl bg-white/5 border border-white/5 hover:border-[#ff9f5c]/20 transition-all group cursor-pointer">
                                                                        <img src={`https://image.tmdb.org/t/p/original${p.logo_path}`} alt={p.provider_name} className="w-10 h-10 rounded-lg shadow-lg" />
                                                                        <div className="flex-1 min-w-0">
                                                                            <div className="text-white font-bold text-sm truncate">{p.provider_name}</div>
                                                        ...
                                                        <div className="bg-[#111118] border border-[#2a2a35] rounded-lg p-6">
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                {providers.buy.map((p, i) => (
                                                                    <a key={i} href={getProviderLink(p.provider_name, anime.title, providers.link)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 rounded-xl bg-white/5 border border-white/5 hover:border-[#5c9fff]/20 transition-all group cursor-pointer">
                                                                        <img src={`https://image.tmdb.org/t/p/original${p.logo_path}`} alt={p.provider_name} className="w-10 h-10 rounded-lg shadow-lg" />
                                                                        <div className="flex-1 min-w-0">
                                                                            <div className="text-white font-bold text-sm truncate">{p.provider_name}</div>
                                            ...
                                            <div className="mt-8 p-5 rounded-2xl bg-black/20 border border-white/5">
                                                <p className="text-[10px] text-[#5c5c6c] font-mono leading-relaxed uppercase tracking-widest text-center">
                                                    Note: Availability varies by region. Deep links will take you directly to search results.
                                                </p>
                                            </div>
                                        </>
                                    );
                    ...
                    <div className="flex flex-col gap-6">
                        {user && myEntry && (
                            ...
                            <div className="bg-[#111118] border border-[#2a2a35] rounded-lg p-5 shadow-sm">
                                ...
                                <div className="flex justify-between py-2.5 gap-4"><span className="font-mono text-[10px] text-[#7a7a90] uppercase tracking-wider flex-shrink-0">Studio</span><span className="font-mono text-[11px] text-white font-bold text-right">{anime.studios || 'TBA'}</span></div>
            ...
            {showAddModal && (
                <AddAnimeModal
                    ...
                    onAdd={async (formData) => {
                        try {
                            await api.post('/anime/log', formData)
