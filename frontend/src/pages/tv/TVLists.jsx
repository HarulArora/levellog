...
const ListItemCard = memo(({ item, onRemove }) => {
    ...
    return (
        <div className="group relative bg-[#111118] border border-[#2a2a35] rounded-xl overflow-hidden hover:border-[#c8ff57] transition-all duration-300">
            <div className="aspect-[3/4] relative overflow-hidden cursor-pointer" onClick={() => navigate(`/tv/${item.externalId}`)}>
            ...
            <div className="p-3">
                ...
                <div className="font-mono text-[9px] text-[#7a7a90] mt-1 uppercase tracking-wider">{item.genre || 'TV Show'}</div>
            </div>
        </div>
    )
})
...
function TVLists() {
    ...
    const handleCreateList = async () => {
        ...
        try {
            await api.post('/lists/custom', { ...newListData, mediaType: 'tv' })
    ...
    const filteredLiked = useMemo(() => likedMovies.filter(m => m.mediaType === 'tv'), [likedMovies])
    const filteredWishlist = useMemo(() => wishlist.filter(m => m.mediaType === 'tv'), [wishlist])
    const filteredLists = useMemo(() => myLists.filter(l => l.mediaType === 'tv'), [myLists])
    ...
    return (
        <div className="max-w-[1200px] mx-auto px-5 md:px-10 py-24 min-h-screen">
            <Helmet>
                <title>My TV Collections | QuestDuck</title>
            </Helmet>
            <SubSectionToggle 
                current="tv"
                type="cinema"
                    ...
                    <div>
                        ...
                        {selectedList.items.length > 0 ? (
                            ...
                            <EmptyState 
                                ...
                                description="Search and add TV shows to this collection from their detail pages."
                                actionLabel="Browse Trending"
                                onAction={() => navigate('/tv/discover')}
                            />
                        )}
                    </div>
                ) : (
                                ...
                                {filteredLists.length === 0 && !loading && (
                                    <div className="mt-8">
                                        <EmptyState 
                                            ...
                                            description="Group your favorite TV shows into custom collections like 'Binge-worthy' or 'Classics'."
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                {filteredLiked.length > 0 ? (
                                    ...
                                    <EmptyState 
                                        ...
                                        description="TV shows you ❤️ will appear here for quick access."
                            ...
                            <div>
                                {filteredWishlist.length > 0 ? (
                                    ...
                                    <EmptyState 
                                        ...
                                        description="Keep track of upcoming seasons or shows you want to watch later."
