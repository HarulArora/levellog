...
function Lists() {
    ...
    const handleCreateList = async () => {
        ...
        try {
            ...
            setCreateError(requiresPro ? 'Upgrade to Pro for unlimited lists' : msg)
        } finally {
            setCreating(false)
        }
    }
    ...
    const handleRemoveFromList = async (listId, igdbId) => {
        try {
            await api.put(`/lists/custom/${listId}/game`, { igdbId, action: 'remove' })
            ...
            {activeTab === 'lists' && (
                <div className="flex flex-col gap-4">
                    ...
                    {customLists?.map(list => (
                        <div key={list._id}
                            className={`bg-[#111118] border rounded-lg transition-all overflow-hidden
                                        ${canCreateList ? 'border-[#2a2a35] hover:border-[#c8ff57]/30' : 'border-[#2a2a35] opacity-70'}`}>
                            {/* Lock banner if level dropped */}
                            {!canCreateList && (
                                <div className="bg-[#ff9f5c]/10 border-b border-[#ff9f5c]/20 px-4 py-2 flex items-center gap-2">
                                    <span className="text-sm">🔒</span>
                                    <span className="font-mono text-[10px] text-[#ff9f5c]">
                                        Locked — reach Level 2 again to edit this list
                                    </span>
                                </div>
                            )}
                            <div className="flex items-center gap-4 p-4">
                                ...
                                <button onClick={() => setShowDeleteConfirm(list._id)}
                                    ...
                                    {list.games.map(game => (
                                        <div key={game.igdbId} className="relative group">
                                            {game.gameCover ? (
                                                ...
                                                <div className="w-12 h-16 bg-[#2a2a35] rounded flex items-center justify-center text-sm">🎮</div>
                                            )}
                                            {canCreateList && (
                                                <button onClick={() => handleRemoveFromList(list._id, game.igdbId)}
                                                    className="absolute -top-1 -right-1 w-4 h-4 bg-[#ff5c5c] rounded-full text-white text-[8px]
                                                               hidden group-hover:flex items-center justify-center">✕</button>
                                            )}
                                        </div>
                                    ))}
                                </div>
                    ...
                    {canCreateList ? (
                        !hasCustomList ? (
                            <button onClick={() => setShowCreateModal(true)}
                                ...
                                <span className="text-lg">+</span> Create Custom List
                            </button>
                        ) : (
                            <div className="w-full py-4 border border-dashed border-[#2a2a35] rounded-lg flex flex-col items-center gap-2 p-5">
                                ...
                                <button className="mt-1 px-4 py-2 bg-[#c8ff57] text-black font-bold text-xs rounded hover:bg-[#d4ff6e] transition-all">
                                    Upgrade to Pro ✨
                                </button>
                            </div>
                        )
                        ...
                        <div className="w-full py-5 border border-dashed border-[#2a2a35] rounded-lg flex flex-col items-center gap-2 p-5">
                            ...
                            <div className="font-mono text-[10px] text-[#7a7a90] text-center max-w-xs">
                                Reach <span className="text-[#c8ff57]">Level 2 (5 XP)</span> to unlock your first custom list.
                ...
                <div className="flex flex-col gap-4">
                        ...
                        {LEVELS.map(lvl => {
                            ...
                            return (
                                <div key={lvl.level}
                                    ...
                                    {isCurrent && (
                                        <span className="font-mono text-[9px] text-[#c8ff57] border border-[#c8ff57]/30 rounded px-2 py-0.5 uppercase tracking-wider">
                                            Current
                                        </span>
                                    )}
                                    {isReached && !isCurrent && (
                                        <span className="font-mono text-[9px] text-[#5c9fff] border border-[#5c9fff]/30 rounded px-2 py-0.5 uppercase tracking-wider">
                                            ✓ Unlocked
                                        </span>
                                    )}
                                </div>
                    ...
                    <div className="bg-[#111118] border border-[#2a2a35] rounded-lg overflow-hidden">
                        ...
                        {[
                            ...
                            <div key={item.action} className="flex items-center gap-3 px-4 py-3 border-b border-[#2a2a35] last:border-0">
                                ...
                                <div className="font-black text-sm text-[#c8ff57]" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
                                    {item.xp}
                                </div>
                            </div>
                        ))}
                        ...
                        <div className="px-4 py-3 bg-[#ff5c5c]/05 border-t border-[#ff5c5c]/10">
                            <div className="font-mono text-[10px] text-[#ff5c5c]">
                                ⚠ Undoing any action deducts the XP it gave
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#111118] border border-[#2a2a35] rounded-lg overflow-hidden">
                        ...
                        {[
                            ...
                            { level: 2, badge: '🕹️', xp: 5, unlock: 'Create 1 Custom List' },
                            ...
                            return (
                                <div key={item.level} className="flex items-center gap-3 px-4 py-3 border-b border-[#2a2a35] last:border-0">
                                    ...
                                    <div className="flex-1">
                                        <div className={`font-mono text-xs ${isReached ? 'text-white' : 'text-[#7a7a90]'}`}>
                                            {item.unlock}
                                        </div>
                                        <div className="font-mono text-[9px] text-[#7a7a90]">
