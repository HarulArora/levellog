...
import { Plus, List as ListIcon, Heart, Bookmark, Search, Trash2, Share2, ChevronRight } from 'lucide-react'
...
const ListItemCard = memo(({ item, onRemove }) => {
    ...
    return (
        <div className="group relative bg-[#111118] border border-[#2a2a35] rounded-xl overflow-hidden hover:border-[#c8ff57] transition-all duration-300">
            <div className="aspect-[3/4] relative overflow-hidden cursor-pointer" onClick={() => navigate(`/manga/${item.externalId}`)}>
            ...
            <div className="p-3">
                ...
                <div className="font-mono text-[9px] text-[#7a7a90] mt-1 uppercase tracking-wider">{item.genre || 'Manga'}</div>
            </div>
        </div>
    )
})
...
function MangaLists() {
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            setLikedAnime(likesRes.data.likedAnime || [])
    ...
    const handleCreateList = async () => {
        ...
        try {
            await api.post('/lists/custom', { ...newListData, mediaType: 'manga' })
    ...
    const handleRemoveFromList = async (listId, externalId) => {
        try {
            ...
            fetchAllData()
            // Update selected list state locally
    ...
    const filteredLiked = useMemo(() => likedAnime.filter(a => a.mediaType === 'manga'), [likedAnime])
    const filteredWishlist = useMemo(() => wishlist.filter(a => a.mediaType === 'manga'), [wishlist])
    const filteredLists = useMemo(() => myLists.filter(l => l.mediaType === 'manga'), [myLists])
    ...
    return (
        <div className="max-w-[1200px] mx-auto px-5 md:px-10 py-24 min-h-screen">
            <Helmet>
                <title>My Manga Collections | QuestDuck</title>
            </Helmet>
            ...
            <SubSectionToggle 
                ...
                type="anime"
                options={[
                    { label: 'Anime', value: 'anime', path: '/anime/lists' },
                    { label: 'Manga', value: 'manga', path: '/manga/lists' }
                ]}
            />
                    ...
                    <div>
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 bg-[#111118] border border-[#2a2a35] p-8 rounded-2xl relative overflow-hidden">
                            ...
                            <div className="flex gap-2 relative z-10">
                                <button className="p-2.5 bg-[#2a2a35] text-white rounded-lg hover:bg-[#3a3a4a] transition-all"><Share2 size={18} /></button>
                        ...
                        {selectedList.items.length > 0 ? (
                            ...
                            <EmptyState 
                                ...
                                description="Search and add manga to this collection from their detail pages."
                                actionLabel="Browse Trending"
                                onAction={() => navigate('/manga/discover')}
                            />
                        )}
                    </div>
                ) : (
                                ...
                                {filteredLists.length === 0 && !loading && (
                                    <div className="mt-8">
                                        <EmptyState 
                                            ...
                                            description="Group your favorite manga into custom collections like 'Hidden Gems' or 'Must Reads'."
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                {filteredLiked.length > 0 ? (
                                    ...
                                    <EmptyState 
                                        ...
                                        description="Manga you ❤️ will appear here for quick access."
                            ...
                            <div>
                                {filteredWishlist.length > 0 ? (
                                    ...
                                    <EmptyState 
                                        ...
                                        description="Keep track of manga you want to read later."
