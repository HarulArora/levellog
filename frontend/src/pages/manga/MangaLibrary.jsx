                                ...
                                <tbody>
                                    {filteredManga.map((manga, idx) => (
                                        <MangaRow 
                                            ...
                                            manga={manga} 
                                            index={idx + 1}
                                            onDelete={() => handleDeleteRequest(manga._id, manga.title)}
            ...
            <td className="px-2 md:px-6 py-3">
                <div className="flex flex-col gap-0.5">
                    ...
                    <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1">
                        ...
                        <p className="text-[#4a4a5e] font-mono text-[8px] md:text-[10px] uppercase tracking-wider truncate max-w-[60px] md:max-w-none">{manga.genre || 'Manga'}</p>
                    </div>
                </div>
            </td>
                ...
                <div className="flex flex-col gap-1 md:gap-2">
                    <div className="flex items-center gap-2">
                        <div className="h-1 flex-1 min-w-[60px] bg-[#1a1a25] rounded-full overflow-hidden">
                            <div 
                                className="h-full bg-[#c8ff57]" 
                                style={{ width: `${Math.min(100, (manga.chaptersRead / (manga.totalChapters || 1)) * 100)}%` }} 
                            />
                        </div>
                        <span className="text-[#7a7a90] font-mono text-[9px] md:text-[10px] whitespace-nowrap">
                            C{manga.chaptersRead || 0}/{manga.totalChapters || '?'}
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="h-1 flex-1 min-w-[60px] bg-[#1a1a25] rounded-full overflow-hidden">
                            <div 
                                className="h-full bg-[#5c9fff]" 
                                style={{ width: `${Math.min(100, (manga.volumesRead / (manga.totalVolumes || 1)) * 100)}%` }} 
                            />
                        </div>
                        <span className="text-[#5c9fff] font-mono text-[9px] md:text-[10px] whitespace-nowrap">
                            V{manga.volumesRead || 0}/{manga.totalVolumes || '?'}
                        </span>
                    </div>
                </div>
