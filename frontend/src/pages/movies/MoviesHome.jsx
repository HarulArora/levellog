                ...
                <div className="flex gap-3 items-start will-change-transform" style={{ animation: `mosaicRight ${isMobile ? '20s' : '32s'} linear infinite`, width: 'max-content' }}>
                    {[...row2Tiles, ...row2Tiles].map((tile, i) => (
                        <img 
                            key={i} 
                            src={tile.img} 
                            alt="Trending Movies Collection" 
                            width={tile.w.match(/\d+/)[0]}
                            height={tile.h.match(/\d+/)[0]}
                            decoding="async"
                            loading="lazy"
...
function MovieSearchBar({ id = 'movie-search', section }) {
    ...
    const handleChange = (e) => {
        ...
        debounceRef.current = setTimeout(async () => {
            try {
                setLoading(true)
                setOpen(true)
                const res = await api.get(`/movies/search?q=${encodeURIComponent(val)}&type=movie&limit=10`)
    ...
    const handleSelect = (item) => {
        ...
        setResults([])
        setOpen(false)
        navigate(`/movies/${item.externalId}`)
    }
        ...
        <div ref={wrapperRef} className="relative w-full">
            <div className="relative">
                ...
                <input
                    id={id}
                    type="text"
                    placeholder={`Search any ${section === 'tv' ? 'TV show' : 'movie'}...`}
                    value={query}
            ...
            {open && (
                <div className="absolute top-[calc(100%+6px)] left-0 right-0 z-[60] bg-[#111118] border border-[#2a2a35] rounded-xl shadow-2xl overflow-hidden">
                    {results.length > 0 ? (
                        ...
                        <div className="px-4 py-5 text-center font-mono text-xs text-[#7a7a90]">No movie found</div>
                    ) : (
...
function MoviesHome() {
    ...
    return (
        <div className="min-h-screen">
            <Helmet>
                <title>QuestDuck | Track Movies</title>
            </Helmet>
            ...
            <div className="md:hidden sticky top-[57px] z-40 bg-[#0d0d14]/95 backdrop-blur-sm border-b border-[#2a2a35] px-4 py-3 flex items-center gap-2">
                <div className="flex-1">
                    <MovieSearchBar id="movie-search-mobile" />
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                        <div>
                            ...
                            <p className="text-[#a0a0b8] text-sm leading-relaxed mb-8 max-w-md">
                                Track everything you watch. From blockbusters to indie gems. 
                                Log movies, rate titles, and see what's trending in cinema.
                            </p>
                                ...
                                {user ? (
                                    <>
                                        <button onClick={() => setShowAddModal(true)} className="btn-apple btn-apple-primary px-6 py-3 gap-1.5">
                                            <Plus size={16} strokeWidth={2.5} /> Log Movie
                                        </button>
                                        <button onClick={() => navigate('/movies/library')} className="btn-apple btn-apple-secondary px-6 py-3 gap-1.5">
                                            My Library →
                                        </button>
                                    </>
                                ) : (
                            ...
                            {user && userMovies.length > 0 ? (
                                <div className="flex gap-8">
                                    {[
                                        ...
                                        { value: userStats.completed, label: 'Watched' },
                                ...
                                <div className="flex gap-8">
                                    {[
                                        { value: '∞', label: 'Movies Indexed' },
                                        { value: 'Free', label: 'Forever' },
                                        { value: 'All', label: 'Studios' },
                                    ].map(stat => (
                                        <div key={stat.label}>
                        ...
                        <div className="hidden md:flex flex-col gap-3">
                            <div className="mb-1">
                                <MovieSearchBar id="movie-search-desktop" />
                            </div>
                            <div className="border-t border-[#2a2a35] my-1" />

                            {recentMovies.length > 0 ? (
                                <>
                                    <div className="font-mono text-[10px] text-[#3a3a4a] uppercase tracking-widest">Recently Watched</div>
                                    {recentMovies.map(item => {
                                        const sc = statusConfig[item.status] || statusConfig.planned
                                        return (
                                            <div key={item._id} onClick={() => navigate(`/movies/${item.externalId}`)} className="flex items-center gap-4 bg-[#111118]/80 border border-[#2a2a35] rounded-lg p-3 hover:border-[#c8ff57]/30 transition-all cursor-pointer">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        ...
                                                        <span className="font-mono text-[9px] uppercase tracking-wider px-2 py-1 rounded-sm bg-[#2a2a35] text-[#94999c]">Movie</span>
                                                    </div>
                                                </div>
                                                {item.rating > 0 && (
                ...
                <div className="max-w-[1200px] mx-auto px-5 md:px-10 py-12">
                    <div className="bg-[#1a1111] border border-red-500/20 rounded-xl p-10 text-center">
                        ...
                        <h3 className="text-white font-black text-2xl uppercase mb-4" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
                            The Movie Pond is Ripple-y
                        </h3>
                        <p className="text-[#7a7a90] text-sm max-w-md mx-auto mb-8 font-mono">
                            We're having trouble reaching the movie database. This is usually a temporary connection reset.
                        </p>
                        <button 
                            onClick={() => refetchHome()}
