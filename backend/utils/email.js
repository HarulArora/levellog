...
if (transporter) {
    logger.info(`🚀 Email Delivery ready via SMTP (${process.env.SMTP_HOST})`)
} else {
    logger.error('❌ No email provider configured. Please check SMTP settings in .env')
}
...
export const sendVerificationEmail = async (email, username, code) => {
    const subject = 'Verify Your QuestDuck Account'
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #0f172a; color: #f8fafc;">
            <h1 style="color: #c8ff57; text-align: center;">Welcome to QuestDuck!</h1>
            <p style="font-size: 16px;">Hello ${username},</p>
            <p style="font-size: 16px;">Thank you for signing up for QuestDuck. To complete your registration, please use the verification code below:</p>
            ...
            <p style="font-size: 12px; color: #64748b; text-align: center;">&copy; ${new Date().getFullYear()} QuestDuck. All rights reserved.</p>
        </div>
    `
...
export const sendResetPasswordEmail = async (email, username, code) => {
    const subject = 'Reset Your QuestDuck Password'
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #0f172a; color: #f8fafc;">
            <h1 style="color: #c8ff57; text-align: center;">Reset Password Request</h1>
            <p style="font-size: 16px;">Hello ${username},</p>
            <p style="font-size: 16px;">We received a request to reset your QuestDuck password. Use the code below to reset it:</p>
            <div style="text-align: center; margin: 30px 0;">
...
export const sendWelcomeEmail = async (email, username) => {
    const subject = 'QuestDuck Account Verified!'
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #0f172a; color: #f8fafc;">
            <h1 style="color: #c8ff57; text-align: center;">You're All Set!</h1>
            <p style="font-size: 16px;">Hello ${username},</p>
            <p style="font-size: 16px;">Your email has been successfully verified. Your QuestDuck account is now fully active!</p>
...
export const sendPasswordResetSuccessEmail = async (email, username) => {
    ...
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #0f172a; color: #f8fafc;">
            ...
            <p style="font-size: 16px;">Hello ${username},</p>
            <p style="font-size: 16px;">This is a confirmation that your QuestDuck password has been changed successfully.</p>
...
export const sendAccountLinkedEmail = async (email, username, provider = 'Google') => {
    const subject = `QuestDuck Account Connected: ${provider}`
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #0f172a; color: #f8fafc;">
            <h1 style="color: #c8ff57; text-align: center;">Account Linked!</h1>
            <p style="font-size: 16px;">Hello ${username},</p>
            <p style="font-size: 16px;">Your QuestDuck profile has been successfully connected to your <b>${provider}</b> account.</p>
...
export const sendAccountUnlinkedEmail = async (email, username, provider = 'Google') => {
    const subject = `QuestDuck Account Disconnected: ${provider}`
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #0f172a; color: #f8fafc;">
            <h1 style="color: #ff5c5c; text-align: center;">Account Disconnected</h1>
            <p style="font-size: 16px;">Hello ${username},</p>
            <p style="font-size: 16px;">This is to confirm that you have disconnected your <b>${provider}</b> account from your QuestDuck profile.</p>
