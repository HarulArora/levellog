...
router.get('/profile/:username', async (req, res) => {
    ...
    try {
        ...
        // Filter out deleted users from followers and following
        const validFollowerIds = await User.find(
            { _id: { $in: user.followers } },
            '_id'
        )
        const validFollowingIds = await User.find(
        ...
        // If stale IDs found, clean them up in the DB silently
        ...
        // Return user with cleaned counts
...
router.post('/follow/:userId', protect, async (req, res) => {
    ...
    try {
        ...
        // ── PRIVATE PROFILE → send request, no XP yet ──
        ...
        if (userToFollow.isPrivate) {
            ...
            return res.json({
                ...
                message: 'Follow request sent · XP awarded on accept',
        ...
        // ── PUBLIC PROFILE → follow directly, +1 XP both sides ──
