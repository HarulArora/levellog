...
// ── GET /api/games/debug/:igdbId ── TEMPORARY DEBUG ROUTE
router.get('/debug/:igdbId', async (req, res) => {
    try {
        const igdbId = req.params.igdbId
        const asNum = Number(igdbId)
        const asStr = String(igdbId)

        const [byNum, byStr, allReviews] = await Promise.all([
            GameReview.find({ igdbId: asNum }),
            GameReview.find({ igdbId: asStr }),
            GameReview.find({}).limit(5)
        ])

        res.json({
            searchedFor: { asNum, asStr },
            foundByNumber: byNum,
            foundByString: byStr,
            sampleAllReviews: allReviews.map(r => ({
                igdbId: r.igdbId,
                igdbIdType: typeof r.igdbId,
                rating: r.rating,
                review: r.review?.slice(0, 30)
            }))
        })
    } catch (err) {
...
router.post('/', protect, async (req, res) => {
    try {
        ...
        res.status(201).json({ success: true, message: 'Game added successfully', game: savedGame })
    } catch (error) {
...
router.put('/:id', protect, async (req, res) => {
    try {
        ...
        if (!hadRatingBefore && hasRatingNow) await awardXP(req.user._id, 1)
        if (existingGame.status !== 'completed' && req.body.status === 'completed') await awardXP(req.user._id, 1)
